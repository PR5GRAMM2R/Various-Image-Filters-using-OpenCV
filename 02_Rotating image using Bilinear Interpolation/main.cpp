...
int main()
{
	...
	for (int i = 0; i < output_x_size * output_y_size; i++)	// 예제...
	{
		...
		Point outputPixelPos(colPosInt, rowPosInt);

		Point inputPixelPos = transposition(rotation(transposition(outputPixelPos, -dest), x), dest);

		pData[i] = getBilinearInterpolation(inputPixelPos, img_in);
	}
